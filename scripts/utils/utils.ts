      ...
      (dirs): Promise<Array<ExerciseApp | ExtraCreditExerciseApp>> => {
        return Promise.all(
          dirs.map(async function getAppFromPath(dir) {
            const relativePath = convertToForwardSlashedPath(
              path.join("exercise", dir),
            );
...
// Many of the functions such as dev/diff assume forward slashes when showing
// or converting or testing paths, so to keep things simple we convert Windows
// paths (typically generated by `path.join`) into forward-slashed paths.
function convertToForwardSlashedPath(text: string) {
